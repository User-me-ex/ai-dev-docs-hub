...
> Specification for durable memory shared across sessions, projects, and agents. This document is normative — implementations MUST satisfy every MUST clause below.
...
- Two-tier storage: relational for metadata, vector for semantic recall.
- Explicit retention and deletion policies.
- Encrypted at rest with per-workspace keys.
...
```mermaid
flowchart LR
  IN([Input]) --> SUB[Persistent Memory]
...
- `memory.write(item)`
- `memory.query(text|filters, k)`
- `memory.delete(id|filters)`
...
- `MemoryRecord { id, workspace, project, agent, kind, text, embedding, tags[], ts, retention }`.

Retention and encryption rules are inherited from [Data Retention](./DATA_RETENTION.md) and [Encryption](./ENCRYPTION.md).

## Failure Modes

- Embedding provider outage → queue writes, backfill on recovery.
