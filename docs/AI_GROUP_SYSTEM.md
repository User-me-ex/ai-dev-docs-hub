...
- `system.spawn_group(group_id, task_id)`
...
- `GroupRun { id, group_id, task_id, workers[], started, ended, verdict }`.

Retention and encryption rules are inherited from [Data Retention](./DATA_RETENTION.md) and [Encryption](./ENCRYPTION.md).

## Failure Modes

- Worker crash loop → circuit breaker opens, Kernel is notified.
