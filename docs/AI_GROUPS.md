...
- `GroupSpec` YAML front matter per group document.
- Kernel routes goals to groups via the Planner.
...
- `Group { id, name, mission, roles[], default_models{}, tools[], kb_scope }`.

Retention and encryption rules are inherited from [Data Retention](./DATA_RETENTION.md) and [Encryption](./ENCRYPTION.md).

## Failure Modes

- Group missing required role → Planner blocks and asks for reassignment.
