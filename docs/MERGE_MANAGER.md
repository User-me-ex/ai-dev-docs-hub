...
> Specification for the concurrency-safe merge coordinator for parallel agent edits. This document is normative — implementations MUST satisfy every MUST clause below.
...
- Serialize conflicting writes without stalling non-conflicting ones.
- Three-way merge with structural awareness for Markdown and code.
- Full audit of every merge decision.
...
- `merge.begin(paths[]) → txn_id`
- `merge.commit(txn_id, patch)`
