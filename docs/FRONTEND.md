...
> Specification for the frontend specification for the reference implementation. This document is normative — implementations MUST satisfy every MUST clause below.
...
- Desktop-first (Tauri) with a browser fallback.
- Zero-config connection to a local backend.
- Every screen documented in [UI_UX](./UI_UX.md).
...
- REST + WebSocket clients generated from [API_SPEC](./API_SPEC.md).
...
- Local cache mirrors backend snapshots; no independent source of truth.

Retention and encryption rules are inherited from [Data Retention](./DATA_RETENTION.md) and [Encryption](./ENCRYPTION.md).

## Failure Modes

- Backend unreachable → offline mode with read-only cache.
